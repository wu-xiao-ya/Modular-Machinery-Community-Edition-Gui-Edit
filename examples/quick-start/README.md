...
- `dynamic-visuals.json`: demonstrates `dynamicVisuals[]`, including multi-source combining, transform-by-value, visibility-by-value, renderer switching, and color interpolation
...
- `dynamic-visuals.json` uses those slider-written keys (`heat`, `warning`) to drive fill, ring, multi-source combining, renderer switching, visibility, color, rotation, and scale in one place
